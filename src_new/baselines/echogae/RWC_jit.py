...
GitHub: https://github.com/faalatawi/echo-chamber-score/blob/main/src/baselines/RWC_jit.py
"""
...
def RWC_JIT(
    ...
    # [2025.07.26 - VD] Add safety checks to avoid division by zero
    e1 = left_left * 1.0 / (left_left + right_left) if (left_left + right_left) > 0 else 0.0
    e2 = left_right * 1.0 / (left_right + right_right) if (left_right + right_right) > 0 else 0.0
    e3 = right_left * 1.0 / (left_left + right_left) if (left_left + right_left) > 0 else 0.0
    e4 = right_right * 1.0 / (left_right + right_right) if (left_right + right_right) > 0 else 0.0
...
def RWC(
    ...
    G: nx.Graph, left_nodes: np.ndarray, right_nodes: np.ndarray, itr_num=1000, percent: float = 0.1
):
